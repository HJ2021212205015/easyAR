...
{
    /// <summary>
    /// <para xml:lang="en">External frame source which represents an external device which has motion tracking capability. Usually represents HMD where camera rendering and device tracking are handled in device SDK.</para>
    ...
    /// <para xml:lang="zh">表示具有运动跟踪能力的外部设备的frame source。通常表示头戴设备，其中相机渲染和设备跟踪都由设备SDK完成。</para>
    /// <para xml:lang="zh">这个frame source是一种运动跟踪设备，在<see cref="ARSession"/>中会输出运动数据。</para>
    /// <para xml:lang="zh">你可以通过继承它来实现头显等设备输入，但你必须自己完成运动跟踪功能，EasyAR的运动跟踪并不能直接在外部设备上使用。你需要自己从硬件或其它地方获取图像和pose数据并输入到EasyAR，EasyAR不提供这些数据的获取能力，但提供将这些数据输入EasyAR之后运行EasyAR功能的能力。</para>
    ...
    {
        private protected override CameraTransformType CameraTransformType => CameraTransformType.SixDof;
        ...
        /// </summary>
        protected bool HandleRenderFrameData(double timestamp, Pose pose, MotionTrackingStatus trackingStatus) => OnRenderFrameData(timestamp, pose, trackingStatus);
        ...
        protected bool HandleCameraFrameData(DeviceFrameSourceCamera deviceCamera, double timestamp, Image image, CameraParameters cameraParameters, Pose deviceToOriginTransform, MotionTrackingStatus trackingStatus)
        {
            ...
            UnityEngine.Debug.LogWarning($"{nameof(HandleCameraFrameData)}: {timestamp}, {trackingStatus}, {deviceToOriginTransform.position}, {deviceToOriginTransform.rotation.eulerAngles}");
#endif
            ...
            var frame = Optional<InputFrame>.Empty;
            if (deviceCamera.AxisSystem == AxisSystemType.Unity)
            {
                var cameraTransform = deviceCamera.Extrinsics.Inverse.GetTransformedBy(deviceToOriginTransform);
                frame = InputFrameHelper.TryCreateSixDof(timestamp, image, cameraParameters, cameraTransform, trackingStatus);
            }
            else if (deviceCamera.AxisSystem == AxisSystemType.EasyAR)
            {
                var cameraTransform = PoseUtility.createPoseWithHardwareOffset(deviceToOriginTransform.position.ToEasyARVector(), deviceToOriginTransform.rotation.ToEasyARQuaternion(), deviceCamera.Extrinsics.Inverse.position.ToEasyARVector(), deviceCamera.Extrinsics.Inverse.rotation.ToEasyARQuaternion()).Value;
                frame = InputFrameHelper.TryCreateSixDof(timestamp, image, cameraParameters, cameraTransform, trackingStatus);
            }
            if (frame.OnNone)
            {
